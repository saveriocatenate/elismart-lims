...
public class LogLogistic3PFitter implements CurveFitter {
    ...
    public CurveParameters fit(List<CalibrationPoint> points) {
        ...
        // 95% CI for EC50 (parameter C, index 1 in [B, C, D]).
        // sigma² rescales the Fisher information matrix to the empirical residual variance.
        int df = xData.length - 3;
        if (df > 0) {
            try {
                RealMatrix cov = optimum.getCovariances(1e-10);
                double sigma2 = optimum.getRMS() * optimum.getRMS() * xData.length / df;
                double seC = Math.sqrt(cov.getEntry(1, 1) * sigma2);
                double tValue = new TDistribution(df).inverseCumulativeProbability(0.975);
                resultParams.put(CurveParameters.META_EC50_LOWER95, fitted[1] - tValue * seC);
                resultParams.put(CurveParameters.META_EC50_UPPER95, fitted[1] + tValue * seC);
            } catch (Exception e) {
                log.warn("3PL: EC50 CI calculation failed (covariance matrix may be singular): {}", e.getMessage());
                resultParams.put(CurveParameters.META_EC50_LOWER95, null);
                resultParams.put(CurveParameters.META_EC50_UPPER95, null);
            }
        } else {
            log.warn("3PL: EC50 CI skipped — insufficient degrees of freedom (n={}, p=3).", xData.length);
            resultParams.put(CurveParameters.META_EC50_LOWER95, null);
            resultParams.put(CurveParameters.META_EC50_UPPER95, null);
        }
